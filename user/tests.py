...
class UserAuthTests(APITestCase):
    ...
    def setUpTestData(cls):
        cls.register_data = [{'email': 'test@test.com',
                              'password': 'pass1234',
                              'fullname': 'Test Test'},
                             {'email': 'test@test.com',
                              'password': 'pass1234',
    ...
    def test_successful_register_user(self):
        ...
        response = self.client.post(url, data=self.register_data[0],
                                    format='json', **self.api_key_headers)
    ...
    def test_existing_register_user(self):
        ...
        response = self.client.post(url, self.register_data[1],
                                    format='json', **self.api_key_headers)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_login_user(self):
        self.client.post(reverse('user_register'), self.register_data[0],
                         format='json', **self.api_key_headers)
        response = self.client.post(reverse('user_login'), self.login_data[0],
                                    format='json', **self.api_key_headers)
    ...
    def test_login_not_existing_user(self):
        self.client.post(reverse('user_register'), self.register_data[0],
                         format='json', **self.api_key_headers)
        response = self.client.post(reverse('user_login'), self.login_data[1],
                                    format='json', **self.api_key_headers)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_logout_user(self):
        token = self.client.post(reverse('user_register'), self.register_data[0],
                                 format='json', **self.api_key_headers).data
        auth_header = {'HTTP_AUTHORIZATION': f'Bearer {token["access"]}'}
        response = self.client.post(reverse('user_logout'),
                                    format='json', **auth_header, **self.api_key_headers)
